...
Three items remain blocked on something outside a code change and are still open: the Google Safe Browsing API key (needs a real Google Cloud credential from the site owner), the first backlink push (needs real usage data plus manual outreach), and the quarterly Search Console review (needs a full quarter of real data and Search Console access).
