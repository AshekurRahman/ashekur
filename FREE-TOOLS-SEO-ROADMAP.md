...
**Update 2026-09-07 (3):** Google Safe Browsing API key — code side is done, two manual steps remain with the site owner. `malware-checker.php` now optionally loads `assets/php/inc/local-secrets.php` (gitignored, template at `local-secrets.example.php`) before reading `GOOGLE_SAFE_BROWSING_API_KEY`, so the real key never has to enter git history regardless of deploy method. The owner supplied a key and it was tested live against Google's official Safe Browsing test URL — the key itself is valid, but the Safe Browsing API is not yet **enabled** on its Google Cloud project (`403 API_KEY_SERVICE_BLOCKED`). Two things still need doing on the owner's side, neither of which is a code change: (1) enable the Safe Browsing API for that key's project at https://console.cloud.google.com/apis/library/safebrowsing.googleapis.com, and (2) copy `local-secrets.example.php` to `local-secrets.php` on the live server (FTP/cPanel File Manager, not git) with the real key filled in.
